...
const { ConnUserCheck, ConnCountUser, ConnSignUp, ConnTokenInsert } = require('../modules/Connection');
        ...
        ConnUserCheck.query(`
            select user_id, ${req.body.login_method}_token as token from user 
                where ${req.body.login_method}_token = '${req.body.user_code}';`, (err, userInfo) => {
                    if(err){
                        console.log('req.body.login_method : ', req.body.login_method);
...
router.post('/user/login', (req, res, next) => {
    ...
    if(req.body.user_id === 0){
        ...
        ConnCountUser.query(`select Max(user_id) as last_user_num from user;`, (err, userCount) => {
            if(err){
...
router.post('/user/login', (req, res, next) => {
    ...
    if(req.body.user_id > 0){
        ...
        ConnTokenInsert.query(`insert login_token (user_id, access_token, refresh_token, refresh_expire) values (?, ?, ?, ?);`, 
